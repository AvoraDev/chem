...
const ELEMENTS = {
  ...
  LoadAll: function(targetDiv, itemStyle = 'none') {
    this.elemKeys.forEach(function(name, index) {
      if (index < 118) {
        ELEMENTS.FormatItem(
          name,
          ELEMENTS.elems[name].number,
          ELEMENTS.elems[name].symbol,
          ELEMENTS.elems[name].mass,
          targetDiv,
          itemStyle
        );
      }
  ...
  Index: function() {
    ...
    // atomic number index
    this.elemKeys.forEach(function(name) {
      ELEMENTS.idx.number.push(ELEMENTS.elems[name].number);
    });
    this.idx.number.sort(function(a, b){return a - b});
    
    // symbol index
    this.elemKeys.forEach(function(name) {
      ELEMENTS.idx.symbol.push(ELEMENTS.elems[name].symbol);
    });
    this.idx.symbol.sort();
    
    // atomic mass index
    this.elemKeys.forEach(function(name) {
      if (typeof(ELEMENTS.elems[name].mass) === 'number') {
        ELEMENTS.idx.mass.push(ELEMENTS.elems[name].mass);
      } else { // i.e. '(286)'
    ...
    this.idx.mass.sort(function(a, b){return a - b});
  },
